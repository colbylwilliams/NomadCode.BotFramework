...
namespace NomadCode.BotFramework
{
	...
	public partial class Conversations : IServiceOperations<DirectLineClient>, IConversations
	{
		...
		/// A response object containing the response body and response headers.
		/// </return>
		public async Task<HttpOperationResponse<ResourceResponse>> UploadWithHttpMessagesLitwareAsync (string conversationId, Stream file, string userId, Activity activity, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default (CancellationToken))
		{
			if (conversationId == null)
			{
				throw new ValidationException (ValidationRules.CannotBeNull, "conversationId");
			...
			{
				StreamContent _file = new StreamContent (file);
				_file.Headers.ContentType = new MediaTypeHeaderValue ("image/jpeg");
				//FileStream _fileAsFileStream = file as FileStream;
				//if (_fileAsFileStream != null)
				//{
				ContentDispositionHeaderValue _contentDispositionHeaderValue = new ContentDispositionHeaderValue ("form-data");
				_contentDispositionHeaderValue.Name = "file";
				_contentDispositionHeaderValue.FileName = "blob";
				_file.Headers.ContentDisposition = _contentDispositionHeaderValue;
				//}
				_multiPartContent.Add (_file, "file");
			}
			...
			if (activity != null)
			{
				var activityJson = Microsoft.Rest.Serialization.SafeJsonConvert.SerializeObject (activity, Client.SerializationSettings);
				var _activity = new StringContent (activityJson, System.Text.Encoding.UTF8);
				_activity.Headers.ContentType = MediaTypeHeaderValue.Parse ("application/vnd.microsoft.activity; charset=utf-8");
				_multiPartContent.Add (_activity, "activity");
			}
